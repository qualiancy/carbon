...
function Base(app, opts) {
  ...
  // TODO: error could be more informative
  var casted = store.schema.cast(opts || {});
  if (!casted.valid) throw casted.errors[0];
  store.set(casted.casted);

  var type = store.get('type');
  state.ns = type + '-state';
  state.debug = sherlock('carbon:balancer:' + type + '-state');
