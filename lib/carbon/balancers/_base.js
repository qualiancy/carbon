...
Base.prototype._drainShards = function(cb) {
  function reset(shard, next) {
    var restart = 'enabled' === shard.state;
    return function() {
      debug('(drain shards) drained: %s', shard.id);
      if (!restart) return next();
      setImmediate(function() {
        debug('(drain shards) re-enable: %s', shard.id);
        shard.enable();
        next();
      });
    }
  ...
  async.forEach(this.shards.toArray(), function(spec, next) {
    ...
    shard.on('disabled', reset(shard, next));
