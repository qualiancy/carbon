...
/**
 * Construct an HTTP type server.
 *
 * Options
 * - `port` _{Number}_ listen on port (required)
 * - `address` _{String}_ listen on host or ip (default: `0.0.0.0`)
 * - `ws` _{Boolean}_ enable websocket proxy (default: `true`)
 *
 * @param {String} name
...
function HTTP(name, _opts) {
  var opts = extend({}, OPTIONS, _opts);
  Base.call(this, 'http', name, opts);
}
...
HTTP.prototype._listen = function(opts, cb) {
  var self = this;
...
HTTP.prototype._close = function(cb) {
  var self = this;
...
HTTP.prototype._request = function(req, res) {
...
HTTP.prototype._upgrade = function(req, socket, head) {
